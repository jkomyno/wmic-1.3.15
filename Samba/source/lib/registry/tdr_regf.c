...
NTSTATUS tdr_pull_regf_hdr (struct tdr_pull *tdr, TALLOC_CTX *mem_ctx, struct regf_hdr *v)
{
	...
	TDR_CHECK(tdr_pull_charset(tdr, mem_ctx, &v->description, 0x40, sizeof(uint16_t), CH_UTF16));
	for (i = 0; i < 83; i++) {
...
NTSTATUS tdr_push_regf_hdr (struct tdr_push *tdr, struct regf_hdr *v)
{
	...
	TDR_CHECK(tdr_push_charset(tdr, &v->description, 0x40, sizeof(uint16_t), CH_UTF16));
	for (i = 0; i < 83; i++) {
...
{
	uint16_t r;
	TDR_CHECK(tdr_pull_uint16(tdr, mem_ctx, &r));
	*v = r;
	return NT_STATUS_OK;
}
...
{
	TDR_CHECK(tdr_push_uint16(tdr, (uint16_t *)v));
	return NT_STATUS_OK;
}
