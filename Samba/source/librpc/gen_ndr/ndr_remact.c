...
static NTSTATUS ndr_push_RemoteActivation(struct ndr_push *ndr, int flags, const struct RemoteActivation *r)
{
	uint32_t cntr_pIIDs_1;
	uint32_t cntr_protseq_0;
	uint32_t cntr_ifaces_1;
	uint32_t cntr_results_1;
	if (flags & NDR_IN) {
		NDR_CHECK(ndr_push_ORPCTHIS(ndr, NDR_SCALARS|NDR_BUFFERS, &r->in.this));
		...
		if (r->in.pwszObjectName) {
			NDR_CHECK(ndr_push_uint32(ndr, NDR_SCALARS, ndr_charset_length(r->in.pwszObjectName, CH_UTF16)));
		...
		NDR_CHECK(ndr_push_uint32(ndr, NDR_SCALARS, r->in.num_protseqs));
	...
	if (flags & NDR_OUT) {
		if (r->out.that == NULL) return NT_STATUS_INVALID_PARAMETER_MIX;
		NDR_CHECK(ndr_push_ORPCTHAT(ndr, NDR_SCALARS|NDR_BUFFERS, r->out.that));
		if (r->out.pOxid == NULL) return NT_STATUS_INVALID_PARAMETER_MIX;
		...
		if (r->out.ServerVersion == NULL) return NT_STATUS_INVALID_PARAMETER_MIX;
		NDR_CHECK(ndr_push_COMVERSION(ndr, NDR_SCALARS, r->out.ServerVersion));
		if (r->out.hr == NULL) return NT_STATUS_INVALID_PARAMETER_MIX;
		NDR_CHECK(ndr_push_WERROR(ndr, NDR_SCALARS, *r->out.hr));
		if (r->out.ifaces == NULL) return NT_STATUS_INVALID_PARAMETER_MIX;
		NDR_CHECK(ndr_push_uint32(ndr, NDR_SCALARS, r->in.Interfaces));
		...
		if (r->out.results == NULL) return NT_STATUS_INVALID_PARAMETER_MIX;
		NDR_CHECK(ndr_push_uint32(ndr, NDR_SCALARS, r->in.Interfaces));
		for (cntr_results_1 = 0; cntr_results_1 < r->in.Interfaces; cntr_results_1++) {
			NDR_CHECK(ndr_push_WERROR(ndr, NDR_SCALARS, r->out.results[cntr_results_1]));
		}
...
static NTSTATUS ndr_pull_RemoteActivation(struct ndr_pull *ndr, int flags, struct RemoteActivation *r)
{
	...
	TALLOC_CTX *_mem_save_pwszObjectName_0;
	TALLOC_CTX *_mem_save_pObjectStorage_0;
	TALLOC_CTX *_mem_save_pIIDs_0;
	...
	TALLOC_CTX *_mem_save_ifaces_2;
	TALLOC_CTX *_mem_save_results_1;
	if (flags & NDR_IN) {
		ZERO_STRUCT(r->out);

		NDR_CHECK(ndr_pull_ORPCTHIS(ndr, NDR_SCALARS|NDR_BUFFERS, &r->in.this));
		...
		NDR_PULL_ALLOC_N(ndr, r->out.results, r->in.Interfaces);
		memset(r->out.results, 0, r->in.Interfaces * sizeof(*r->out.results));
		if (r->in.pIIDs) {
	...
	if (flags & NDR_OUT) {
		...
		_mem_save_results_1 = NDR_PULL_GET_MEM_CTX(ndr);
		NDR_PULL_SET_MEM_CTX(ndr, r->out.results, 0);
		for (cntr_results_1 = 0; cntr_results_1 < r->in.Interfaces; cntr_results_1++) {
			NDR_CHECK(ndr_pull_WERROR(ndr, NDR_SCALARS, &(r->out.results)[cntr_results_1]));
		}
		NDR_PULL_SET_MEM_CTX(ndr, _mem_save_results_1, 0);
...
_PUBLIC_ void ndr_print_RemoteActivation(struct ndr_print *ndr, const char *name, int flags, const struct RemoteActivation *r)
{
	...
	if (flags & NDR_IN) {
		...
		ndr->depth++;
		ndr_print_ORPCTHIS(ndr, "this", &r->in.this);
		...
		ndr->depth++;
		if (r->in.pIIDs) {
			ndr->print(ndr, "%s: ARRAY(%d)", "pIIDs", r->in.Interfaces);
			ndr->depth++;
			for (cntr_pIIDs_1=0;cntr_pIIDs_1<r->in.Interfaces;cntr_pIIDs_1++) {
				char *idx_1=NULL;
				asprintf(&idx_1, "[%d]", cntr_pIIDs_1);
				if (idx_1) {
		...
		ndr->print(ndr, "%s: ARRAY(%d)", "protseq", r->in.num_protseqs);
		ndr->depth++;
		for (cntr_protseq_0=0;cntr_protseq_0<r->in.num_protseqs;cntr_protseq_0++) {
			char *idx_0=NULL;
			asprintf(&idx_0, "[%d]", cntr_protseq_0);
			if (idx_0) {
	...
	if (flags & NDR_OUT) {
		...
		ndr->depth++;
		ndr->print(ndr, "%s: ARRAY(%d)", "ifaces", r->in.Interfaces);
		ndr->depth++;
		for (cntr_ifaces_1=0;cntr_ifaces_1<r->in.Interfaces;cntr_ifaces_1++) {
			char *idx_1=NULL;
			asprintf(&idx_1, "[%d]", cntr_ifaces_1);
			if (idx_1) {
		...
		ndr->depth++;
		ndr->print(ndr, "%s: ARRAY(%d)", "results", r->in.Interfaces);
		ndr->depth++;
		for (cntr_results_1=0;cntr_results_1<r->in.Interfaces;cntr_results_1++) {
			char *idx_1=NULL;
			asprintf(&idx_1, "[%d]", cntr_results_1);
			if (idx_1) {
				ndr_print_WERROR(ndr, "results", r->out.results[cntr_results_1]);
				free(idx_1);
			}
		}
		ndr->depth--;
...
static const struct dcerpc_interface_call IRemoteActivation_calls[] = {
	{
		"RemoteActivation",
...
static const struct dcerpc_endpoint_list IRemoteActivation_endpoints = {
	.count	= 3,
...
static const struct dcerpc_authservice_list IRemoteActivation_authservices = {
	.count	= 3,
	.names	= IRemoteActivation_authservice_strings
};


const struct dcerpc_interface_table dcerpc_table_IRemoteActivation = {
	.name		= "IRemoteActivation",
	.syntax_id	= {
		{0x4d9f4ab8,0x7d1c,0x11cf,{0x86,0x1e},{0x00,0x20,0xaf,0x6e,0x7c,0x57}},
		DCERPC_IREMOTEACTIVATION_VERSION
	},
	.helpstring	= DCERPC_IREMOTEACTIVATION_HELPSTRING,
	.num_calls	= 1,
