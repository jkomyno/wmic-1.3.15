...
static NTSTATUS ndr_pull_netr_DsRGetSiteName(struct ndr_pull *ndr, int flags, struct netr_DsRGetSiteName *r)
{
	...
	if (flags & NDR_OUT) {
		...
		if (r->out.site) {
			...
			NDR_PULL_SET_MEM_CTX(ndr, _mem_save_site_0, 0);
		}
		NDR_CHECK(ndr_pull_WERROR(ndr, NDR_SCALARS, &r->out.result));
	}
	return NT_STATUS_OK;
...
static NTSTATUS ndr_push_netr_DsrEnumerateDomainTrusts(struct ndr_push *ndr, int flags, const struct netr_DsrEnumerateDomainTrusts *r)
{
	...
	if (flags & NDR_OUT) {
		...
		NDR_CHECK(ndr_push_unique_ptr(ndr, r->out.trusts));
		if (r->out.trusts) {
			NDR_CHECK(ndr_push_uint32(ndr, NDR_SCALARS, r->out.count));
			for (cntr_trusts_1 = 0; cntr_trusts_1 < r->out.count; cntr_trusts_1++) {
				NDR_CHECK(ndr_push_netr_DomainTrust(ndr, NDR_SCALARS, &r->out.trusts[cntr_trusts_1]));
			}
			for (cntr_trusts_1 = 0; cntr_trusts_1 < r->out.count; cntr_trusts_1++) {
				NDR_CHECK(ndr_push_netr_DomainTrust(ndr, NDR_BUFFERS, &r->out.trusts[cntr_trusts_1]));
			}
		}
		NDR_CHECK(ndr_push_WERROR(ndr, NDR_SCALARS, r->out.result));
	}
...
static NTSTATUS ndr_pull_netr_DsrEnumerateDomainTrusts(struct ndr_pull *ndr, int flags, struct netr_DsrEnumerateDomainTrusts *r)
{
	...
	if (flags & NDR_OUT) {
		...
		if (r->out.trusts) {
			...
			NDR_PULL_SET_MEM_CTX(ndr, r->out.trusts, 0);
			for (cntr_trusts_1 = 0; cntr_trusts_1 < r->out.count; cntr_trusts_1++) {
				NDR_CHECK(ndr_pull_netr_DomainTrust(ndr, NDR_SCALARS, &(r->out.trusts)[cntr_trusts_1]));
			}
			for (cntr_trusts_1 = 0; cntr_trusts_1 < r->out.count; cntr_trusts_1++) {
				NDR_CHECK(ndr_pull_netr_DomainTrust(ndr, NDR_BUFFERS, &(r->out.trusts)[cntr_trusts_1]));
			}
			NDR_PULL_SET_MEM_CTX(ndr, _mem_save_trusts_1, 0);
			NDR_PULL_SET_MEM_CTX(ndr, _mem_save_trusts_0, 0);
		}
...
static const struct dcerpc_interface_call netlogon_calls[] = {
	{
		"netr_LogonUasLogon",
...
static const struct dcerpc_endpoint_list netlogon_endpoints = {
	.count	= 3,
...
static const struct dcerpc_authservice_list netlogon_authservices = {
	.count	= 3,
