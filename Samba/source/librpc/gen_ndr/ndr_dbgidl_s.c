...
static NTSTATUS dbgidl__op_bind(struct dcesrv_call_state *dce_call, const struct dcesrv_interface *iface)
{
...
static NTSTATUS dbgidl__op_ndr_pull(struct dcesrv_call_state *dce_call, TALLOC_CTX *mem_ctx, struct ndr_pull *pull, void **r)
{
	...
	if (opnum >= dcerpc_table_dbgidl.num_calls) {
		dce_call->fault_code = DCERPC_FAULT_OP_RNG_ERROR;
		return NT_STATUS_NET_WRITE_FAULT;
	}

	*r = talloc_size(mem_ctx, dcerpc_table_dbgidl.calls[opnum].struct_size);
	NT_STATUS_HAVE_NO_MEMORY(*r);

        /* unravel the NDR for the packet */
	status = dcerpc_table_dbgidl.calls[opnum].ndr_pull(pull, NDR_IN, *r);
	if (!NT_STATUS_IS_OK(status)) {
		dcerpc_log_packet(&dcerpc_table_dbgidl, opnum, NDR_IN,
...
static NTSTATUS dbgidl__op_dispatch(struct dcesrv_call_state *dce_call, TALLOC_CTX *mem_ctx, void *r)
{
	...
	case 0: {
		...
		dummy_dbgidl(dce_call, mem_ctx, r2);
...
static NTSTATUS dbgidl__op_ndr_push(struct dcesrv_call_state *dce_call, TALLOC_CTX *mem_ctx, struct ndr_push *push, const void *r)
{
	...
	status = dcerpc_table_dbgidl.calls[opnum].ndr_push(push, NDR_OUT, r);
	if (!NT_STATUS_IS_OK(status)) {
		dce_call->fault_code = DCERPC_FAULT_NDR;
		return NT_STATUS_NET_WRITE_FAULT;
	}
...
static const struct dcesrv_interface dbgidl_interface = {
	.name		= "dbgidl",
...
static NTSTATUS dbgidl__op_init_server(struct dcesrv_context *dce_ctx, const struct dcesrv_endpoint_server *ep_server)
{
	...
	for (i=0;i<dcerpc_table_dbgidl.endpoints->count;i++) {
		NTSTATUS ret;
		const char *name = dcerpc_table_dbgidl.endpoints->names[i];

		ret = dcesrv_interface_register(dce_ctx, name, &dbgidl_interface, NULL);
...
{
	if (dbgidl_interface.syntax_id.if_version == if_version &&
		GUID_equal(&dbgidl_interface.syntax_id.uuid, uuid)) {
		memcpy(iface,&dbgidl_interface, sizeof(*iface));
		return True;
	}
...
{
	if (strcmp(dbgidl_interface.name, name)==0) {
		memcpy(iface,&dbgidl_interface, sizeof(*iface));
		return True;
	}
