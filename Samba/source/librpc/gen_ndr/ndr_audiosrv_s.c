...
static NTSTATUS audiosrv__op_bind(struct dcesrv_call_state *dce_call, const struct dcesrv_interface *iface)
{
...
static NTSTATUS audiosrv__op_ndr_pull(struct dcesrv_call_state *dce_call, TALLOC_CTX *mem_ctx, struct ndr_pull *pull, void **r)
{
	...
	if (opnum >= dcerpc_table_audiosrv.num_calls) {
		dce_call->fault_code = DCERPC_FAULT_OP_RNG_ERROR;
		return NT_STATUS_NET_WRITE_FAULT;
	}

	*r = talloc_size(mem_ctx, dcerpc_table_audiosrv.calls[opnum].struct_size);
	NT_STATUS_HAVE_NO_MEMORY(*r);

        /* unravel the NDR for the packet */
	status = dcerpc_table_audiosrv.calls[opnum].ndr_pull(pull, NDR_IN, *r);
	if (!NT_STATUS_IS_OK(status)) {
		dcerpc_log_packet(&dcerpc_table_audiosrv, opnum, NDR_IN,
...
static NTSTATUS audiosrv__op_dispatch(struct dcesrv_call_state *dce_call, TALLOC_CTX *mem_ctx, void *r)
{
	...
	case 0: {
		...
		audiosrv_CreatezoneFactoriesList(dce_call, mem_ctx, r2);
	...
	case 1: {
		...
		audiosrv_CreateGfxFactoriesList(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
	...
	case 2: {
		...
		audiosrv_CreateGfxList(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
	...
	case 3: {
		...
		audiosrv_RemoveGfx(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
	...
	case 4: {
		...
		audiosrv_AddGfx(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
	...
	case 5: {
		...
		audiosrv_ModifyGfx(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
	...
	case 6: {
		...
		audiosrv_OpenGfx(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
	...
	case 7: {
		...
		audiosrv_Logon(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
	...
	case 8: {
		...
		audiosrv_Logoff(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
	...
	case 9: {
		...
		audiosrv_RegisterSessionNotificationEvent(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
	...
	case 10: {
		...
		audiosrv_UnregisterSessionNotificationEvent(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
	...
	case 11: {
		...
		audiosrv_SessionConnectState(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
	...
	case 12: {
		...
		audiosrv_DriverOpenDrvRegKey(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
	...
	case 13: {
		...
		audiosrv_AdvisePreferredDeviceChange(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
	...
	case 14: {
		...
		audiosrv_GetPnpInfo(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
...
static NTSTATUS audiosrv__op_ndr_push(struct dcesrv_call_state *dce_call, TALLOC_CTX *mem_ctx, struct ndr_push *push, const void *r)
{
	...
	status = dcerpc_table_audiosrv.calls[opnum].ndr_push(push, NDR_OUT, r);
	if (!NT_STATUS_IS_OK(status)) {
		dce_call->fault_code = DCERPC_FAULT_NDR;
		return NT_STATUS_NET_WRITE_FAULT;
	}
...
static const struct dcesrv_interface audiosrv_interface = {
	.name		= "audiosrv",
...
static NTSTATUS audiosrv__op_init_server(struct dcesrv_context *dce_ctx, const struct dcesrv_endpoint_server *ep_server)
{
	...
	for (i=0;i<dcerpc_table_audiosrv.endpoints->count;i++) {
		NTSTATUS ret;
		const char *name = dcerpc_table_audiosrv.endpoints->names[i];

		ret = dcesrv_interface_register(dce_ctx, name, &audiosrv_interface, NULL);
...
{
	if (audiosrv_interface.syntax_id.if_version == if_version &&
		GUID_equal(&audiosrv_interface.syntax_id.uuid, uuid)) {
		memcpy(iface,&audiosrv_interface, sizeof(*iface));
		return True;
	}
...
{
	if (strcmp(audiosrv_interface.name, name)==0) {
		memcpy(iface,&audiosrv_interface, sizeof(*iface));
		return True;
	}
