...
struct dcerpc_syntax_id_p {
	struct dcerpc_syntax_id *id;/* [unique] */
};

struct rpc_if_id_vector_t {
	uint32_t count;
	struct dcerpc_syntax_id_p *if_id;/* [size_is(count)] */
};
...
struct mgmt_inq_if_ids {
	struct {
		struct rpc_if_id_vector_t *if_id_vector;/* [unique] */
		WERROR result;
	} out;
...
struct mgmt_inq_princ_name {
	...
	struct {
		const char *princ_name;/* [string,charset(DOS)] */
		WERROR result;
	} out;
