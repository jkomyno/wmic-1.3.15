...
static NTSTATUS unixinfo__op_bind(struct dcesrv_call_state *dce_call, const struct dcesrv_interface *iface)
{
...
static NTSTATUS unixinfo__op_ndr_pull(struct dcesrv_call_state *dce_call, TALLOC_CTX *mem_ctx, struct ndr_pull *pull, void **r)
{
	...
	if (opnum >= dcerpc_table_unixinfo.num_calls) {
		dce_call->fault_code = DCERPC_FAULT_OP_RNG_ERROR;
		return NT_STATUS_NET_WRITE_FAULT;
	}

	*r = talloc_size(mem_ctx, dcerpc_table_unixinfo.calls[opnum].struct_size);
	NT_STATUS_HAVE_NO_MEMORY(*r);

        /* unravel the NDR for the packet */
	status = dcerpc_table_unixinfo.calls[opnum].ndr_pull(pull, NDR_IN, *r);
	if (!NT_STATUS_IS_OK(status)) {
		dcerpc_log_packet(&dcerpc_table_unixinfo, opnum, NDR_IN,
...
static NTSTATUS unixinfo__op_dispatch(struct dcesrv_call_state *dce_call, TALLOC_CTX *mem_ctx, void *r)
{
	...
	case 0: {
		...
		r2->out.result = unixinfo_SidToUid(dce_call, mem_ctx, r2);
	...
	case 1: {
		...
		r2->out.result = unixinfo_UidToSid(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
	...
	case 2: {
		...
		r2->out.result = unixinfo_SidToGid(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
	...
	case 3: {
		...
		r2->out.result = unixinfo_GidToSid(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
	...
	case 4: {
		...
		r2->out.result = unixinfo_GetPWUid(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
...
static NTSTATUS unixinfo__op_ndr_push(struct dcesrv_call_state *dce_call, TALLOC_CTX *mem_ctx, struct ndr_push *push, const void *r)
{
	...
	status = dcerpc_table_unixinfo.calls[opnum].ndr_push(push, NDR_OUT, r);
	if (!NT_STATUS_IS_OK(status)) {
		dce_call->fault_code = DCERPC_FAULT_NDR;
		return NT_STATUS_NET_WRITE_FAULT;
	}
...
static const struct dcesrv_interface unixinfo_interface = {
	.name		= "unixinfo",
...
static NTSTATUS unixinfo__op_init_server(struct dcesrv_context *dce_ctx, const struct dcesrv_endpoint_server *ep_server)
{
	...
	for (i=0;i<dcerpc_table_unixinfo.endpoints->count;i++) {
		NTSTATUS ret;
		const char *name = dcerpc_table_unixinfo.endpoints->names[i];

		ret = dcesrv_interface_register(dce_ctx, name, &unixinfo_interface, NULL);
...
{
	if (unixinfo_interface.syntax_id.if_version == if_version &&
		GUID_equal(&unixinfo_interface.syntax_id.uuid, uuid)) {
		memcpy(iface,&unixinfo_interface, sizeof(*iface));
		return True;
	}
...
{
	if (strcmp(unixinfo_interface.name, name)==0) {
		memcpy(iface,&unixinfo_interface, sizeof(*iface));
		return True;
	}
