...
static NTSTATUS w32time__op_dispatch(struct dcesrv_call_state *dce_call, TALLOC_CTX *mem_ctx, void *r)
{
	...
	case 0: {
		...
		r2->out.result = w32time_SyncTime(dce_call, mem_ctx, r2);
	...
	case 1: {
		...
		r2->out.result = w32time_GetNetLogonServiceBits(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
	...
	case 2: {
		...
		r2->out.result = w32time_QueryProviderStatus(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
...
static const struct dcesrv_interface w32time_interface = {
	.name		= "w32time",
...
static NTSTATUS w32time__op_init_server(struct dcesrv_context *dce_ctx, const struct dcesrv_endpoint_server *ep_server)
{
	...
	for (i=0;i<dcerpc_table_w32time.endpoints->count;i++) {
		...
		ret = dcesrv_interface_register(dce_ctx, name, &w32time_interface, NULL);
...
{
	if (w32time_interface.syntax_id.if_version == if_version &&
		GUID_equal(&w32time_interface.syntax_id.uuid, uuid)) {
		memcpy(iface,&w32time_interface, sizeof(*iface));
		return True;
	}
...
{
	if (strcmp(w32time_interface.name, name)==0) {
		memcpy(iface,&w32time_interface, sizeof(*iface));
		return True;
	}
