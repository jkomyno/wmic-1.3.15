...
static NTSTATUS rot__op_bind(struct dcesrv_call_state *dce_call, const struct dcesrv_interface *iface)
{
...
static NTSTATUS rot__op_ndr_pull(struct dcesrv_call_state *dce_call, TALLOC_CTX *mem_ctx, struct ndr_pull *pull, void **r)
{
	...
	if (opnum >= dcerpc_table_rot.num_calls) {
		dce_call->fault_code = DCERPC_FAULT_OP_RNG_ERROR;
		return NT_STATUS_NET_WRITE_FAULT;
	}

	*r = talloc_size(mem_ctx, dcerpc_table_rot.calls[opnum].struct_size);
	NT_STATUS_HAVE_NO_MEMORY(*r);

        /* unravel the NDR for the packet */
	status = dcerpc_table_rot.calls[opnum].ndr_pull(pull, NDR_IN, *r);
	if (!NT_STATUS_IS_OK(status)) {
		dcerpc_log_packet(&dcerpc_table_rot, opnum, NDR_IN,
...
static NTSTATUS rot__op_dispatch(struct dcesrv_call_state *dce_call, TALLOC_CTX *mem_ctx, void *r)
{
	...
	case 0: {
		...
		r2->out.result = rot_add(dce_call, mem_ctx, r2);
	...
	case 1: {
		...
		r2->out.result = rot_remove(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
	...
	case 2: {
		...
		r2->out.result = rot_is_listed(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
	...
	case 3: {
		...
		r2->out.result = rot_get_interface_pointer(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
	...
	case 4: {
		...
		r2->out.result = rot_set_modification_time(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
	...
	case 5: {
		...
		r2->out.result = rot_get_modification_time(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
	...
	case 6: {
		...
		r2->out.result = rot_enum(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
...
static NTSTATUS rot__op_ndr_push(struct dcesrv_call_state *dce_call, TALLOC_CTX *mem_ctx, struct ndr_push *push, const void *r)
{
	...
	status = dcerpc_table_rot.calls[opnum].ndr_push(push, NDR_OUT, r);
	if (!NT_STATUS_IS_OK(status)) {
		dce_call->fault_code = DCERPC_FAULT_NDR;
		return NT_STATUS_NET_WRITE_FAULT;
	}
...
static const struct dcesrv_interface rot_interface = {
	.name		= "rot",
...
static NTSTATUS rot__op_init_server(struct dcesrv_context *dce_ctx, const struct dcesrv_endpoint_server *ep_server)
{
	...
	for (i=0;i<dcerpc_table_rot.endpoints->count;i++) {
		NTSTATUS ret;
		const char *name = dcerpc_table_rot.endpoints->names[i];

		ret = dcesrv_interface_register(dce_ctx, name, &rot_interface, NULL);
...
{
	if (rot_interface.syntax_id.if_version == if_version &&
		GUID_equal(&rot_interface.syntax_id.uuid, uuid)) {
		memcpy(iface,&rot_interface, sizeof(*iface));
		return True;
	}
...
{
	if (strcmp(rot_interface.name, name)==0) {
		memcpy(iface,&rot_interface, sizeof(*iface));
		return True;
	}
