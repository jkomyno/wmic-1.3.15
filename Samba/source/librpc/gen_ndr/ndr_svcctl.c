...
static NTSTATUS ndr_pull_svcctl_GetServiceKeyNameW(struct ndr_pull *ndr, int flags, struct svcctl_GetServiceKeyNameW *r)
{
	...
	if (flags & NDR_OUT) {
		...
		if (r->out.key_name) {
			...
			NDR_PULL_SET_MEM_CTX(ndr, _mem_save_key_name_0, 0);
		}
		NDR_CHECK(ndr_pull_generic_ptr(ndr, &_ptr_display_name_length));
		if (_ptr_display_name_length) {
			NDR_PULL_ALLOC(ndr, r->out.display_name_length);
...
static const struct dcerpc_interface_call svcctl_calls[] = {
	{
...
static const struct dcerpc_endpoint_list svcctl_endpoints = {
	.count	= 2,
...
static const struct dcerpc_authservice_list svcctl_authservices = {
	.count	= 2,
